...
//divide total rounds ROUNDS=100 by the number of processes
int ROUNDS_PER_PROCESS = ROUNDS / size_proc;


//each process executes ROUNDS_PER_PROCESS rounds
for (i = 0; i < ROUNDS_PER_PROCESS; i++) 
   {
...
/*we divided ROUNDS=100 by the number of ranks/processes 
 *for each process to execute. still remainder 
 *(ROUNDS % number of ranks/processes) rounds need to be 
 *executed
*/

/* (ROUNDS % number of ranks/processes) will always be 
 * less than the number of ranks/processes. we can divide
 * this remainder rounds across ranks where each rank less than
 * the remainder executes 1 round.
*/

if(rank_id < (ROUNDS % size_proc)) { pi = dboard(DARTS); avepi = avepi + pi; ROUNDS_PER_PROCESS+=1;}
 ...
 *one proess performs ROUNDS_PER_PROCESS rounds.
*/
double pi_est_here = avepi / (ROUNDS_PER_PROCESS);
printf("PI Estimate is %f for rank  %d \n",pi_est_here, rank_id);


/*this is what is reduced to the root rank
 *to calculate the average pi value across all
 *rounds and across all processes
*/
double pi_proc = avepi / (ROUNDS);   


printf("%d rounds executed by process %d \n",ROUNDS_PER_PROCESS, rank_id);
...
printf("final estimate is %f after time elapsed of %f \n",reduced_val,time_elapsed);
}
